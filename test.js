 ...
 * bezier-smooth.js
 * ─────────────────────────────────────────────────────────────────────────────
 * Pre-processing pass for the weese/transit-map pipeline.
 *
 * WHAT IT DOES
 * ============
 * After the MIP optimiser has placed every node on an octilinear grid and
 * before svg-transit-map renders lines as straight polylines, this module:
 *
 *  1. Traces each transit line through the graph in order.
 *  2. At every "bend" (direction change) **on a non-interchange station** it
 *     inserts two **virtual control points** that are offset from the real
 *     station along the incoming and outgoing segment directions.  These
 *     control points become the handles of a cubic Bézier curve.
 *     Interchange stations (passed by ≥ 2 lines) are **never smoothed**:
 *     they remain at their exact optimised grid coordinates so that transfer
 *     markers are visually unambiguous.
 *  3. Moves the *station marker* position to the point on the Bézier curve
 *     closest to the original grid coordinate so station dots remain visually
 *     on the line.
 *  4. Returns a new graph object (nodes + edges) that svg-transit-map (or any
 *     compatible renderer) can consume directly.
 *
 * USAGE
 * =====
 *  const smooth = require('./bezier-smooth')
 *
 *  // graph  – JSON Graph Format object with { nodes, edges }
 *  // lineMap – Map<lineId, orderedNodeIdArray>   (see buildLineMap below)
 *  const smoothed = smooth(graph, lineMap, options)
 *
 * OPTIONS
 * =======
 *  tension      {number}  0–1  Controls how far the control points are pulled
 *                              away from the station.  0 = no curve, 1 = very
 *                              rounded.  Default 0.35.
 *  minBendDeg   {number}  Minimum angle change (degrees) before smoothing is
 *                              applied.  Straights are left alone.  Default 15.
 *  remapStations {boolean} If true, remap each real station's x/y to the
 *                              nearest point on its smoothed curve segment.
 *                              Default true.
 *
 * FORMAT NOTES
 * ============
 * Input  graph.nodes[]  { id, metadata: { x, y, label? } }
 * Input  graph.edges[]  { source, target, metadata: { lines: [id, …] } }
 * Output: same format, with additional virtual nodes injected and station
 *         coordinates potentially adjusted.  Virtual nodes are tagged with
 *         metadata.virtual = true so the renderer can draw them as invisible
 *         waypoints rather than station circles.
 */
...
module.exports = {
  ...
  analyzeLoopCorners,
  computeLoopTensions,
  // geometry utils exposed for testing
  cubicBezier,
  closestTOnBezier,
  vecSub,
  vecAdd,
